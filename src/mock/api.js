...
export const createMockApi = (publicClient) => {
            ...
            logs.forEach((log) => {
                try {
                    ...
                    if ('cluster' in event.args) {
                        clusterSnapshots.set(createClusterId(event.args.owner, event.args.operatorIds.map(Number)), {
    ...
    return {
        getOwnerNonce: vi.fn().mockImplementation((args) => Promise.resolve({
        ...
        getCluster: vi.fn().mockImplementation((args) => Promise.resolve({
            cluster: {
                active: true,
        ...
        getClusters: vi.fn().mockImplementation(() => Promise.resolve({
        ...
        getOperators: vi.fn().mockImplementation(() => Promise.resolve({
        ...
        getValidators: vi.fn().mockImplementation(() => Promise.resolve({
            validators: Array.from(operators.values()).map((operator) => ({
                id: operator.publicKey,
            })),
        })),
        getValidator: vi.fn().mockImplementation((args) => {
            const validator = Array.from(operators.values()).find((op) => op.publicKey === args.validatorId);
            return Promise.resolve({
                validator: validator
                    ? {
