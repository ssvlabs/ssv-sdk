...
export class KeyShares {
  ...
  static async fromJson(content: string | any): Promise<KeyShares> {
    ...
    if (
      !extVersion ||
      currentVersion.major !== extVersion.major ||
      currentVersion.minor !== extVersion.minor
    ) {
      throw new SSVKeysException(
        `The keyshares file you are attempting to reuse does not have the same version (v${pkg.version}) as supported by ssv-keys`,
      );
    }
