...
export type Contracts = {
  setter: {
    ...
    writeFnNames: ContractFunctionName<
      typeof MainnetV4SetterABI,
      'nonpayable' | 'payable'
    >;
    writeFunctions: ExtractAbiFunctions<
      typeof MainnetV4SetterABI,
      'nonpayable' | 'payable'
    >;
    readFnNames: ContractFunctionName<
      typeof MainnetV4SetterABI,
      'view' | 'pure'
    >;
    readFunctions: ExtractAbiFunctions<
      typeof MainnetV4SetterABI,
      'view' | 'pure'
    >;
  };
...
export type WriterFunctions<
  ...
  Fns extends
    Contracts[ContractName]['writeFunctions'] = Contracts[ContractName]['writeFunctions'],
> = {
  [K in Fns as K['name']]: K['inputs'] extends readonly []
    ? {
        (
          props?: Prettify<
            (K['stateMutability'] extends 'payable'
              ? { value?: bigint }
              : object) &
              WriteOptions<K>
          >,
...
export type ReaderFunctions<
  ...
  Fns extends
    Contracts[ContractName]['readFunctions'] = Contracts[ContractName]['readFunctions'],
> = {
  [K in Fns as K['name']]: K['inputs'] extends readonly []
    ? () => Promise<
        ReadContractReturnType<
