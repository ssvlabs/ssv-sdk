...
export class EncryptedKeyValidatorConstraint
  ...
  validate(value: any) {
    ...
    try {
      ...
      encryptedKeys.forEach((key: any) => {
        ...
        util.decode64(
          key.startsWith('0x')
            ? decodeAbiParameters([{ type: 'string' }], key)[0]
            : key,
        );
      });
    } catch (e: any) {
      throw new KeySharesAbiDecodeError(
        keyWithError,
        `Filed ABI decode shares encrypted key. Error: ${e.message}`,
      );
    }
