...
describe('KeyShares.fromJson/toJson', () => {
  ...
  it('Should create empty data with version', async () => {
    ...
    expect(keySharesObject.data.operators).toEqual(
      mockKeySharesItemEmpty.data.operators,
    );
    expect(keySharesObject.data.publicKey).toEqual(
      mockKeySharesItemEmpty.data.publicKey,
    );
  ...
  it('Should throw an error for invalid operator data', async () => {
    expect(
      (await KeySharesItem.fromJson(mockKeySharesItemWithMissedOperatorKey))
        .error,
    ).toBeInstanceOf(OperatorsCountsMismatchError);
    expect(
      (await KeySharesItem.fromJson(mockKeySharesItemWithWrongOperatorId))
        .error,
    ).toBeInstanceOf(OperatorsCountsMismatchError);
    expect(
      (await KeySharesItem.fromJson(mockKeySharesItemWithDuplicatedOperatorKey))
        .error,
    ).toBeInstanceOf(DuplicatedOperatorPublicKeyError);
  });

  it('Should create KeyShares Item', async () => {
    expect(
      async () => await KeySharesItem.fromJson(mockKeySharesItemWithOperators),
    ).not.toThrowError();
  });
  ...
  it('should throw error on version incompatibility', async () => {
    ...
    await expect(KeyShares.fromJson(incompatibleVersionJson)).rejects.toThrow(
      /does not have the same version.*as supported by ssv-keys/,
    );
  });
