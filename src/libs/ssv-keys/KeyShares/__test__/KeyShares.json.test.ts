...
describe('KeyShares.fromJson/toJson', () => {
  ...
  it('Should throw an error for invalid operator data', async () => {
    expect(
      (await KeySharesItem.fromJson(mockKeySharesItemWithMissedOperatorKey)).error,
    ).toBeInstanceOf(OperatorsCountsMismatchError)
    expect(
      (await KeySharesItem.fromJson(mockKeySharesItemWithWrongOperatorId)).error,
    ).toBeInstanceOf(OperatorsCountsMismatchError)
    expect(
      (await KeySharesItem.fromJson(mockKeySharesItemWithDuplicatedOperatorKey)).error,
