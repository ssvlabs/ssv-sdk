...
describe('Cluster Methods', () => {
  describe('registerValidators', () => {
    it('should register a single validator', async () => {
      const { registerValidators } = await import(
    ...
    it('should throw error when exceeding validator limit', async () => {
      ...
      await expect(
        ...
        "You can't register more than 10 validators in a single transaction",
      );
    });
    ...
    it('should throw error when no validator added events found', async () => {
      ...
      await expect(
        validateSharesPostRegistration(mockConfig, {
          txHash: '0x123' as Hex,
        }),
  ...
  describe('removeValidators', () => {
    it('should remove validators', async () => {
      const { removeValidators } = await import(
        '../libs/cluster/methods/remove-validators'
      );
      ...
      expect(result).toBeDefined();
      expect(
        mockConfig.contract.ssv.write.removeValidator,
      ).toHaveBeenCalledWith({
        args: {
          cluster: mockCluster,
          operatorIds: mockOperatorIdsBigInt,
  ...
  describe('exitValidators', () => {
    it('should exit validators', async () => {
      const { exitValidators } = await import(
        '../libs/cluster/methods/exit-validators'
      );
  ...
  describe('liquidateCluster', () => {
    it('should liquidate a cluster', async () => {
      const { liquidateCluster } = await import(
        '../libs/cluster/methods/liquidate-cluster'
      );
  ...
  describe('reactivateCluster', () => {
    it('should reactivate a cluster', async () => {
      const { reactivateCluster } = await import(
        '../libs/cluster/methods/reactivate-cluster'
      );
  ...
  describe('setFeeRecipient', () => {
    it('should set fee recipient', async () => {
      const { setFeeRecipient } = await import(
        '../libs/cluster/methods/set-fee-recipient'
      );
      ...
      expect(result).toBeDefined();
      expect(
        mockConfig.contract.ssv.write.setFeeRecipientAddress,
      ).toHaveBeenCalledWith({
        args: {
