...
const createMockOperator = (
  ...
  id: mockOperatorId,
  publicKey:
    '0x1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890' as Hex,
  validatorCount: '0',
  isPrivate: false,
  whitelisted: [],
...
describe('Operator Methods', () => {
  ...
  describe('withdraw', () => {
    it('should withdraw partial earnings', async () => {
      ...
      expect(result).toBeDefined();
      expect(
        mockConfig.contract.ssv.read.getOperatorEarnings,
      ).toHaveBeenCalledWith({
        id: 1n,
      });
      expect(
        mockConfig.contract.ssv.write.withdrawOperatorEarnings,
      ).toHaveBeenCalledWith({
        args: {
          operatorId: 1n,
          amount: 1000n,
  ...
  describe('setOperatorWhitelists', () => {
    beforeEach(() => {
      vi.mocked(
        mockConfig.contract.ssv.read.isWhitelistingContract,
      ).mockResolvedValue(true);
    });

    it('should set operator whitelists for valid contract', async () => {
      const { setOperatorWhitelists } = await import(
        '../libs/operator/methods'
      );
      ...
      expect(
        mockConfig.contract.ssv.write.setOperatorsWhitelists,
      ).toHaveBeenCalledWith({
        args: {
          operatorIds: [1n],
    ...
    it('should throw error for invalid contract', async () => {
      ...
      await expect(
        setOperatorWhitelists(mockConfig, {
          args: {
            operatorIds: [mockOperatorId],
            contractAddress: mockContractAddress,
          },
  ...
  describe('canAccountUseOperator', () => {
    beforeEach(() => {
      vi.mocked(
        mockConfig.contract.ssv.read.isAddressWhitelistedInWhitelistingContract,
      ).mockResolvedValue(true);
    });

    it('should return true for public operator', async () => {
      const { canAccountUseOperator } = await import(
        '../libs/operator/methods'
      );

      const result = await canAccountUseOperator(
        mockConfig,
        createMockOperator(),
        mockAccount,
      );
