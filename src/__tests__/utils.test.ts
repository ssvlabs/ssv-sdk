...
describe('URL Join Utils', () => {
  ...
  test('handles query parameters correctly', () => {
    expect(urlJoin('http://example.com', '?query=1')).toBe(
  ...
  test('handles hash fragments correctly', () => {
    expect(urlJoin('http://example.com', '#hash')).toBe(
      'http://example.com#hash',
    );
    expect(urlJoin('http://example.com/', '#hash')).toBe(
      'http://example.com#hash',
    );
    expect(urlJoin('http://example.com', 'path', '#hash')).toBe(
  ...
  test('handles multiple path segments', () => {
    ...
    expect(
      urlJoin('http://example.com/', '/path1/', '/path2/', '/path3/'),
    ).toBe('http://example.com/path1/path2/path3/');
  });
...
describe('Keyshares Utils', () => {
  ...
  test('validateConsistentOperatorIds validates operator IDs correctly', () => {
    ...
    expect(() =>
      validateConsistentOperatorIds([createMockKeyshare(), invalidKeyshare]),
  ...
  test('ensureValidatorsUniqueness validates validator uniqueness', () => {
    ...
    expect(() => ensureValidatorsUniqueness(duplicateKeyshares)).toThrow(
      KeysharesValidationError,
    );
  });
  ...
  test('validateConsistentOperatorPublicKeys validates operator public keys', () => {
    ...
    expect(
      validateConsistentOperatorPublicKeys([createMockKeyshare()], operators),
    ).toBe(true);
  ...
  test('ensureNoKeysharesErrors checks for keyshare errors', () => {
    ...
    const keyshareWithError = createMockKeyshare({
      error: new Error('Test error'),
    });
    expect(() => ensureNoKeysharesErrors([keyshareWithError])).toThrow(
      'Test error',
    );
  });
});
...
describe('BigInt Utils', () => {
  ...
  test('isBigIntChanged detects changes beyond tolerance', () => {
    expect(isBigIntChanged(parseEther('0.0001'), parseEther('0.0003'))).toBe(
      true,
    );
...
describe('Cluster Utils', () => {
  test('createClusterId creates valid cluster ID', () => {
    ...
    expect(() => createClusterId('invalid', operatorIds)).toThrow(
  ...
  test('isClusterId validates cluster IDs correctly', () => {
    ...
    const invalidOperators =
      '0x1234567890123456789012345678901234567890-1-2-3-invalid';
