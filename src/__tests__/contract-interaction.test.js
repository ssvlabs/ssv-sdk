...
describe('SSV Keys', async () => {
    ...
    it('can write to the SSVNetwork contract', async () => {
        ...
        const operatorAddedEvent = receipt.events.find((event) => event.eventName === 'OperatorAdded');
        expect(operatorAddedEvent).toBeDefined();
        expect(operatorAddedEvent.args.operatorId).toBe(1n);
        expect(decodeOperatorPublicKey(operatorAddedEvent.args.publicKey)).toBe(mockFetchedOperators[0].publicKey);
        expect(operatorAddedEvent?.args.fee).toBe(blockFee);
        expect(operatorAddedEvent?.args.owner).toBe(getAddress(sdk.config.walletClient.account.address));
    });
