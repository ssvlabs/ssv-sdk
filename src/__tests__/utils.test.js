import { bigintAbs, bigintFloor, bigintifyNumbers, bigintMax, bigintMin, bigintRound, isBigIntChanged, roundOperatorFee, stringifyBigints, } from '@/utils/bigint';
import { add0x, createClusterId, createEmptyCluster, getClusterSnapshot, isClusterId, } from '@/utils/cluster';
import { ensureNoKeysharesErrors, ensureValidatorsUniqueness, isKeySharesItem, KeysharesValidationError, validateConsistentOperatorIds, validateConsistentOperatorPublicKeys, } from '@/utils/keyshares';
...
describe('URL Join Utils', () => {
    ...
    test('handles query parameters correctly', () => {
        ...
        expect(urlJoin('http://example.com/', '?query=1')).toBe('http://example.com?query=1');
        expect(urlJoin('http://example.com', 'path', '?query=1')).toBe('http://example.com/path?query=1');
    });
    ...
    test('handles multiple path segments', () => {
        expect(urlJoin('http://example.com', 'path1', 'path2', 'path3')).toBe('http://example.com/path1/path2/path3');
        expect(urlJoin('http://example.com/', '/path1/', '/path2/', '/path3/')).toBe('http://example.com/path1/path2/path3/');
    });
...
describe('Keyshares Utils', () => {
    const createMockKeyshare = (overrides = {}) => {
        return {
            data: {
                publicKey: 'pubkey1',
                operators: [
    ...
    test('validateConsistentOperatorIds validates operator IDs correctly', () => {
        ...
        expect(() => validateConsistentOperatorIds([createMockKeyshare(), invalidKeyshare])).toThrow(KeysharesValidationError);
    });
    ...
    test('validateConsistentOperatorPublicKeys validates operator public keys', () => {
        ...
        expect(() => validateConsistentOperatorPublicKeys([createMockKeyshare()], invalidOperators)).toThrow(KeysharesValidationError);
    });
...
describe('BigInt Utils', () => {
    ...
    test('bigintFloor floors correctly', () => {
        ...
        expect(bigintFloor(15000000n, precision)).toBe(10000000n);
        expect(bigintFloor(25000000n, precision)).toBe(20000000n);
    });
    ...
    test('roundOperatorFee rounds operator fee correctly', () => {
        const precision = 10000000n;
        expect(roundOperatorFee(15000000n, precision)).toBe(20000000n);
        expect(roundOperatorFee(14000000n, precision)).toBe(10000000n);
    });
...
describe('Cluster Utils', () => {
    test('createClusterId creates valid cluster ID', () => {
        ...
        expect(createClusterId(ownerAddress, operatorIds)).toBe('0x1234567890123456789012345678901234567890-1-2-3-4');
