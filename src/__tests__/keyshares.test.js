...
const mockConfig = {
    ...
    api: {
        ...
        getOperators: vi.fn().mockResolvedValue(mockFetchedOperators.map((o) => ({
            id: String(o.id),
...
describe('Keyshares', async () => {
    ...
    const sdk = new SSVSDK(createMockConfig({
        addresses: {
    ...
    it('should throw for invalid nonce', async () => {
        ...
        await expect(validateSharesPreRegistration(invalidNonceConfig, {
            operatorIds: mockOperators.ids.map(String),
            keyshares: valid_keyshares,
    ...
    it('should throw for no nonce', async () => {
        ...
        await expect(validateSharesPreRegistration(noNonceConfig, {
            operatorIds: mockOperators.ids.map(String),
            keyshares: valid_keyshares,
    ...
    it('should throw if every validator is registered', async () => {
        ...
        await expect(validateSharesPreRegistration(registeredValidatorsConfig, {
            operatorIds: mockOperators.ids.map(String),
            keyshares: valid_keyshares,
        })).rejects.toThrow();
    });
    it('should throw for invalid operator key', async () => {
        await expect(sdk.utils.validateKeysharesJSON({
    ...
    it('should throw when operator is private and account is not whitelisted', async () => {
        ...
        await expect(validateSharesPreRegistration(privateOperatorConfig, {
            operatorIds: mockOperators.ids.map(String),
            keyshares: valid_keyshares,
        })).rejects.toThrow();
    });
    it('should throw when operator has reached max validator count', async () => {
        ...
        await expect(validateSharesPreRegistration(maxedOutOperators, {
            operatorIds: mockOperators.ids.map(String),
            keyshares: valid_keyshares,
        })).rejects.toThrow();
    });
