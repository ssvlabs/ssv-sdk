...
const keys = require("./SSVKeys-C22o0Fby.js");
...
const fs = keys.require$$2;
const path = keys.require$$2;
const os = keys.require$$2;
const crypto = keys.requireCryptoBrowserify();
...
function _dotenvKey(options2) {
  ...
  if (index.process$1.env.DOTENV_KEY && index.process$1.env.DOTENV_KEY.length > 0) {
    return index.process$1.env.DOTENV_KEY;
  }
  return "";
}
...
function _vaultPath(options2) {
  ...
  if (options2 && options2.path && options2.path.length > 0) {
    ...
    possibleVaultPath = path.resolve(index.process$1.cwd(), ".env.vault");
  }
...
function _configVault(options2) {
  ...
  let processEnv = index.process$1.env;
...
function configDotenv(options2) {
  const dotenvPath = path.resolve(index.process$1.cwd(), ".env");
  let encoding = "utf8";
  const debug2 = Boolean(options2 && options2.debug);
  const quiet = options2 && "quiet" in options2 ? options2.quiet : true;
  ...
  if (debug2 || !quiet) {
    ...
    for (const filePath of optionPaths) {
      try {
        const relative = path.relative(index.process$1.cwd(), filePath);
...
function decrypt(encrypted, keyStr) {
  const key = keys.Buffer.from(keyStr.slice(-64), "hex");
  let ciphertext = keys.Buffer.from(encrypted, "base64");
...
const options = {};
if (index.process$1.env.DOTENV_CONFIG_ENCODING != null) {
  options.encoding = index.process$1.env.DOTENV_CONFIG_ENCODING;
}
if (index.process$1.env.DOTENV_CONFIG_PATH != null) {
  options.path = index.process$1.env.DOTENV_CONFIG_PATH;
}
if (index.process$1.env.DOTENV_CONFIG_QUIET != null) {
  options.quiet = index.process$1.env.DOTENV_CONFIG_QUIET;
}
if (index.process$1.env.DOTENV_CONFIG_DEBUG != null) {
  options.debug = index.process$1.env.DOTENV_CONFIG_DEBUG;
}
if (index.process$1.env.DOTENV_CONFIG_OVERRIDE != null) {
  options.override = index.process$1.env.DOTENV_CONFIG_OVERRIDE;
}
if (index.process$1.env.DOTENV_CONFIG_DOTENV_KEY != null) {
  options.DOTENV_KEY = index.process$1.env.DOTENV_CONFIG_DOTENV_KEY;
}
...
(function() {
  mainExports.config(
    Object.assign(
      {},
      envOptions,
      cliOptions(index.process$1.argv)
    )
  );
})();
...
index.process$1.env.NODE_ENV === "production";
const instanceOf = (
  /* c8 ignore next 6 */
...
const deposit = async (config2, { args: { id, amount }, ...writeOptions }, options2 = {}) => {
  ...
  return config2.contract.ssv.write.deposit({
    args: {
      amount,
      cluster: snapshot,
      clusterOwner: index.process$1.env.OWNER_ADDRESS,
...
const ssvKeys$1 = new keys.SSVKeys();
const validateSharesPostRegistration = async (config2, args) => {
  ...
  const validations = [];
  for (const [index2, e] of validatorAddedEvents.entries()) {
    validations.push({
      event: e,
      validation: await ssvKeys$1.validateSharesPostRegistration({
        ...
        ownerNonce: Number(ownerNonce) + index2,
...
const debug$1 = typeof index.process$1 === "object" && index.process$1.env && index.process$1.env.NODE_DEBUG && /\bsemver\b/i.test(index.process$1.env.NODE_DEBUG) ? (...args) => console.error("SEMVER", ...args) : () => {
};
var debug_1 = debug$1;
...
var eq_1 = eq;
const equalVersion = /* @__PURE__ */ keys.getDefaultExportFromCjs(eq_1);
...
var valid_1 = valid;
const validVersion = /* @__PURE__ */ keys.getDefaultExportFromCjs(valid_1);
...
const _KeyShares = class _KeyShares2 {
  ...
  static async fromJson(content) {
    ...
    if (!validVersion(body.version)) {
      throw new keys.SSVKeysException(
    ...
    if (Array.isArray(body.shares)) {
      for (const item of body.shares) {
        instance.shares.push(await keys.KeySharesItem.fromJson(item));
      }
    } else {
      instance.shares.push(await keys.KeySharesItem.fromJson(body));
    }
    return instance;
  }
};
...
const ssvKeys = new keys.SSVKeys();
...
const generateKeyShares = async (args) => {
  ...
  for (let i = 0; i < keystores.length; i++) {
    ...
    const operators = args.operator_keys.map((key, index2) => ({
      id: args.operator_ids[index2],
      operatorKey: key
    }));
    ...
    shares.push(
      await new keys.KeySharesItem().buildPayload(
        {
