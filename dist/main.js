...
const getOwnerNonce = (client, args) => {
  const document = typeof args.block === "number" ? globals.GetOwnerNonceByBlockDocument : globals.GetOwnerNonceDocument;
  return client.request(document, args).then((r) => r.account?.nonce || "0").catch(() => "0");
};
const getClusterSnapshot = (client, args) => client.request(globals.GetClusterSnapshotDocument, args).then((res) => res.cluster);
const getCluster = (client, args) => client.request(globals.GetClusterDocument, args).then((res) => res.cluster);
const getClusters = (client, args) => client.request(globals.GetClustersDocument, args).then((res) => res.clusters);
const getOperator = (client, args) => client.request(globals.GetOperatorDocument, args).then((res) => {
  if (!res.operator)
    return null;
  return {
    ...res.operator,
...
const getOperators = (client, args) => client.request(globals.GetOperatorsDocument, args).then((res) => res.operators.map((o) => ({
  ...o,
  publicKey: globals.decodeOperatorPublicKey(o.publicKey),
  whitelisted: o.whitelisted.map((v) => v.id)
})));
const getValidators = (client, args) => client.request(globals.GetValidatorsDocument, args).then((res) => res.validators);
const getValidator = (client, args) => client.request(globals.GetValidatorDocument, args).then((res) => res.validator);
const getClusterBalance$1 = (client, args) => client.request(globals.GetClusterBalanceDocument, args);
...
const paramsToArray = ({ params, abiFunction }) => {
  return globals.stringifyBigints(abiFunction.inputs.reduce((acc, param) => {
...
const createConfig = (props) => {
  ...
  const graphQLClient = new graphqlRequest.GraphQLClient(graphEndpoint);
  return {
    publicClient: parsed.publicClient,
    walletClient: parsed.walletClient,
...
const registerValidators = async (config2, { args: { keyshares, depositAmount = 0n }, ...writeOptions }) => {
  ...
  if (!limit) {
    throw new Error(`Invalid number of operators in keyshares: ${clusterSize}, should be one of: ${Object.keys(globals.registerValidatorsByClusterSizeLimits).join(", ")}`);
  }
...
const canAccountUseOperator = async (config2, operator, account) => {
  ...
  if (!operator.isPrivate)
    return true;
  const isWhitelisted = operator.whitelisted.some((addr) => viem.isAddressEqual(addr, account));
  if (isWhitelisted)
    return true;
  const hasExternalContract = Boolean(operator.whitelistedContract && operator.whitelistedContract !== viem.zeroAddress);
...
class KeyShares {
  ...
  static async fromJson(content) {
    ...
    if (!validVersion(body.version)) {
      throw new KeySharesItem.SSVKeysException(`The file for keyshares must contain a version mark provided by ssv-keys.`);
    }
    if (!equalVersion(body.version, "v1.1.0") && !equalVersion(body.version, "v1.2.0")) {
      throw new KeySharesItem.SSVKeysException(`The keyshares file you are attempting to reuse does not have the same version (v1.1.0 or v1.2.0) as supported by ssv-keys`);
    }
    const instance = new KeyShares();
    instance.shares = [];
    if (Array.isArray(body.shares)) {
      for (const item of body.shares) {
        instance.shares.push(await KeySharesItem.KeySharesItem.fromJson(item));
      }
    } else {
      instance.shares.push(await KeySharesItem.KeySharesItem.fromJson(body));
    }
    return instance;
  }
}
...
const generateKeyShares = async (args) => {
  ...
  for (let i = 0; i < keystores.length; i++) {
    ...
    shares.push(await new KeySharesItem.KeySharesItem().buildPayload({
